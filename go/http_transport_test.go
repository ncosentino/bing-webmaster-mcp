...
func TestHTTPTransport_ServesRealSession(t *testing.T) {
	...
	server := newServer(
		bingwebmaster.NewClient("test-key"),
		indexnow.NewClient("test-indexnow-key"),
	)
	...
	if len(tools.Tools) != 55 {
		t.Errorf("tools = %d, want 55", len(tools.Tools))
	}
}
...
func TestHTTPTransport_ServesHealth(t *testing.T) {
	...
	if health.Status != "ok" || health.Service != "bing-webmaster-mcp" {
